function SL2P_DASF_uniform(varargin)

%% 1. Initialization
if ~ismember(nargin,[2,3]), disp({'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ERROR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!';'--usage : Matlab SL2P_DASF_uniform [input_path\] [S2 tiff data folder] [output_path\ (optional)]'});return; end;
...
if nargin==3,   
    out_path=[varargin{3},strrep(varargin{2},'L2A','L2B'),'_SL2P_DASF_uniform\'];
    else,
        out_path=[varargin{1},strrep(varargin{2},'L2A','L2B'),'_SL2P_DASF_uniform\'];
end;
...
%% 3. Loading NET
disp({'--Loading NNET--------------------------------------'});
NET=importdata('aux_data\S2_SL2P_D_uniform_NET.mat');
NET_uncer=importdata('aux_data\S2_SL2P_D_uniform_uncert_NET.mat');
%% 4 Computing input_flags 
...
for ivar=1:length(bio_vars),
    ...
    for d = 1:length(NET.Plist),
        dindex = find((D>(NET.Plist(d)+NET.Plist(max(1,d-1)))/2)&(D<=(NET.Plist(d)+NET.Plist(min(length(NET.Plist),d+1)))/2));
        NNT_IN_D = NNT_IN(:,dindex); 
        
        eval(['NET_D= NET.D.',bio,'.NETS(',num2str(d),').NET;']);
        bio_sim (dindex,1)= sim(NET_D, NNT_IN_D)';
    end;
    %% add flags data
